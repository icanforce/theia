...
if __name__ == '__main__':
    ...
    if not os.path.isfile(dataFile): 
            ...
            with open(dataFile, 'wb') as fdat:
                pickle.dump(
                    (
                        ...
                        nData, gt, gt_mi, gt_m, putative, corr, ppi, aux
                    ),
                    fdat
                )  
    else:
        ...
        # miRNA, mRNA, nInput, nOutput, miRNA_list, mRNA_list, nData, gt, gt_mi, gt_m, putative, corr, ppi, aux = pickle.load( open( dataFile, "rb" ) )
    ...
    # mi, m = module.find(miRNA_list, mRNA_list, U, V, nInput, nOutput, params['K'], th=0.5) # 1 or 0
    ...
    #draw.plot_heatmap(params, S,U,V, S, prefix_aux='SNMNMF_')
    
    # corr_th = 0.0
    # gt_sign = ((corr*gt)<(-corr_th))*(-1) + ((corr*gt)>corr_th)*(1)
    # p, r, f, a, tp, fp, tn, fn = score.calcScore(S*W, gt_sign, 0.1, putative)
    
    # print p, r, f
    
    # for th in [0.001, 0.01, 0.1, 0.2, 0.3]:
        # detection_rate = score.calcRecall(S*W, gt_sign, 0.01)
