...
from numpy import copysign, mean, ones, set_printoptions, var, fromiter
...
def run(signal, noise, thresh=0):
    ...
    modules = find_modules(miRNA_list, mRNA_list, U, V, 1) 
    #print(modules)
    estimation = SW * putative
...
def run_grid():
    ...
    with Pool() as p:
        f_results, ari_results = zip(*p.starmap(run, zip(s.reshape(s.size), n.reshape(n.size))))
        f_results = fromiter(f_results, dtype=float).reshape(s.shape)
        ari_results = fromiter(ari_results, dtype=float).reshape(s.shape)
    ...
    dirname = 'PIMIM_synthetic_results'
...
fresults, ariresults, presults, rresults = zip(*(run(0.1, 1.0, thresh=thresh) for thresh in arange(0, 1.0, 0.1)))
print(presults)
print(rresults)
